...
void spi_set_speed(spi_port sp, const uint32_t uiPortSpeed)
{
  ...
  if (ret == -1)
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "Error setting SPI speed.");
}
...
void spi_set_mode(spi_port sp, const uint32_t uiPortMode)
{
  ...
  if (ret == -1)
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "Error setting SPI mode.");
}
...
spi_get_speed(spi_port sp)
{
  ...
  if (ret == -1)
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "Error reading SPI speed.");
