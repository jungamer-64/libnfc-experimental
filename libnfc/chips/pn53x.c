...
static int
pn53x_transceive_bytes_with_retries(struct nfc_device *pnd,
                                    const uint8_t *cmd,
                                    const size_t cmd_len,
                                    const int timeout,
                                    const uint8_t max_failures)
{
  int ret = 0;
  uint8_t failures = 0;
  ...
  {
    ret = nfc_initiator_transceive_bytes(pnd, cmd, cmd_len, NULL, 0, timeout);
    if (ret < 1)
    {
                                      ...
                                      const uint8_t *pbtRx, bool *mi)
{
  ...
  case TgSetMetaData:
    if (pbtRx[0] & 0x80)
    {
      abort(); // NAD detected - protocol violation
    }
    *mi = pbtRx[0] & 0x40; // Message Incomplete flag
                             ...
                             nfc_target_info *pnti)
{
  ...
  case NMT_ISO14443A:
    ...
    if (szRawData > (pnti->nai.szUidLen + 5))
    {
      ...
      if (copy_result < 0)
      {
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR, "ATS copy failed: buffer overflow prevented");
        return NFC_ECHIP;
      }
    }
    else
  ...
  case NMT_ISO14443BI:
    ...
    if (pnti->nii.btVerLog & 0x80)
    {
      ...
      {
        // Safe copy ATR data, variable size (szRawData - 8)
  ...
  case NMT_ISO14443B2SR:
    // Store the UID - Safe copy, fixed 8 bytes
  ...
  case NMT_ISO14443B2CT:
    // Store UID LSB - Safe copy, fixed 2 bytes
    ...
    // Store UID MSB - Safe copy with offset +2, fixed 2 bytes
  ...
  case NMT_FELICA:
    ...
    // Copy the NFCID2t - Safe copy, fixed 8 bytes
    if (nfc_safe_memcpy(pnti->nfi.abtId, sizeof(pnti->nfi.abtId), pbtRawData, 8) < 0)
      return NFC_ECHIP;
    pbtRawData += 8;
    // Copy the felica padding - Safe copy, fixed 8 bytes
    ...
    {
      // Safe copy system code, fixed 2 bytes
...
static int pn53x_ISO14443A_4_is_present(struct nfc_device *pnd)
{
  ...
  else if (CHIP_DATA(pnd)->type == PN532)
  {
    ...
    if ((ret = pn53x_set_property_bool(pnd, NP_EASY_FRAMING, false)) < 0)
      return ret;
    const uint8_t abtCmd[] = {0xb2}; // CID=0
    ret = pn53x_transceive_bytes_with_retries(pnd, abtCmd, sizeof(abtCmd), 300, 2);
    int ret2;
...
static int pn53x_ISO14443A_Jewel_is_present(struct nfc_device *pnd)
{
  ...
  const uint8_t abtCmd[] = {0x78};
  return pn53x_transceive_bytes_with_retries(pnd, abtCmd, sizeof(abtCmd), -1, 2);
}
...
static int pn53x_ISO14443A_MFUL_is_present(struct nfc_device *pnd)
{
  ...
  else
  {
    ...
    ret = pn53x_transceive_bytes_with_retries(pnd, abtCmd, sizeof(abtCmd), -1, 2);
  }
  return ret;
}
...
static int pn53x_ISO14443B_4_is_present(struct nfc_device *pnd)
{
  ...
  else
  {
    ...
    const uint8_t abtCmd[] = {0xba, 0x01}; // if on PN532, CID=1
    ret = pn53x_transceive_bytes_with_retries(pnd, abtCmd, sizeof(abtCmd), 300, 2);
    int ret2;
    if ((ret2 = pn53x_set_property_bool(pnd, NP_EASY_FRAMING, true)) < 0)
      ret = ret2;
...
static int pn53x_ISO14443B_SR_is_present(struct nfc_device *pnd)
{
  ...
  return pn53x_transceive_bytes_with_retries(pnd, abtCmd, sizeof(abtCmd), 300, 2);
}
