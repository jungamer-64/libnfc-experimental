...
pn71xx_open(const nfc_context *context, const nfc_connstring connstring)
{
  ...
  // Safe copy device name (fixed string copy with constant length)
  /* "pn71xx-device" length is constant 13 */
  if (nfc_safe_memcpy(pnd->name, sizeof(pnd->name),
                      "pn71xx-device", 14) < 0) {
    /* 13 + 1 for null terminator */
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
  ...
  // Safe copy connection string with length check (CWE-126)
  size_t connstring_len = nfc_safe_strlen(connstring, sizeof(pnd->connstring));
  if (nfc_safe_memcpy(pnd->connstring, sizeof(pnd->connstring),
                      connstring, connstring_len + 1) < 0) {
    log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_ERROR,
            "Failed to copy connection string");
    nfc_device_free(pnd);
    return NULL;
...
pn71xx_get_information_about(nfc_device *pnd, char **pbuf)
{
  ...
  /* Constant string length is 53 */
  const size_t buflen = 54; /* 53 + 1 for null terminator */
