...
acr122s_open(const nfc_context *context, const nfc_connstring connstring)
{
  ...
  pnd->driver = &acr122s_driver;
  /* Safely determine driver name length (CWE-126) */
  size_t driver_name_len = nfc_safe_strlen(ACR122S_DRIVER_NAME, DEVICE_NAME_LENGTH);
...
const struct nfc_driver acr122s_driver = {
    ...
    PN53X_DRIVER_STATE_FUNCTIONS(acr122s_abort_command, pn53x_idle, NULL)
};
