...
現状メモ: `ffi-sanity` は `examples/ffi-sanity/ffi-sanity.c` をソースにしたテスト専用ターゲットとして扱い、2026-03-31 時点では GitHub Actions に常設 job としてはまだ接続されていない。
...
- step-3: Build the C library and the test-only `ffi-sanity` target against the generated Rust staticlib.
- step-4: Run the small integration test set behind `ffi-sanity` to exercise basic APIs (context create/free, basic read/write, log callback roundtrip).
